...
Laser mount adjustable via two screws, height and position perpendicular to optical axis. The angle wrt the optical axis is adjusted by rotating the laser by hand.

The cavity, or better said the mirrors, are adjustable via four screws, two per mirror. One vertical, one horizontal, that is, the angle wrt to the plane perpendicular to the optical axis.

Green laser is non adjustable, continuous, not pulsed, easily visible with bare eyes.

PMT detector is connected to a power supply which has one knob to adjust its sensitivity, the other cable goes to the oscilloscope that reads the detector signal.
...
starting with 532 nm alignment laser and alignment completely screwed up.
first we try adjusting the laser position so it goes straight along the optical axis through the first two pinholes, keeping them mostly closed to see where the laser point lands, with the goal of hitting the third pinhole (entry of cavity) perfectly in the middle.
then next we adjust the four screws of the mirrors (one vertical and one horizontal per mirror) until the green laser exits through the other end of the cavity.
we hold a piece of paper towards the exit hole to make sure it exits right in the center, so it hits the PMT detector centrally.
since the mirrors almost completely allow transmission of the green light we can see the laser exiting, as opposed to the blue laser (loses most of its intensity upon entering the cavity), so thats why we use the green laser, to adjust the alignment of the mirrors.

next we exchange the green alignment laser with blue laser. now since the mirrors are theoretically well aligned, all we need to do is adjust the laser position so that it goes through the first two pinholes and hits the third pinhole centrally. this is hard as the blue laser is not very intense but its possible to roughly adjust so that it hits the cavity entry pinhole.

then we screw the detector on the exit end of the cavity and turn on its power supply. then next we adjust the four mirror screws while looking at the intensity on the oscilloscope.
