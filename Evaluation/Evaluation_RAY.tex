...
For each trace, the data points in the fit window $t\in[\SI{102}{\micro\second},\,\SI{125}{\micro\second}]$ were transformed to $(t,\ln y)$ and fitted with an ordinary least-squares (OLS) straight-line model.
The uncertainty of the slope $b$ was obtained from the covariance matrix returned by the OLS fit.
...
From the measured ring-down times, the molecular scattering coefficient is obtained using Eq.~\eqref{eq:beta_from_taus}.
...
\[
\overline{\beta}=\frac{1}{5}\sum_{i=1}^{5}\beta_i=(7.910\pm 3.002)\times 10^{-5}\,\si{m^{-1}}.
\]
